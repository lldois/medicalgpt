...
def main():
    ...
    if args.resize_emb and base_model.get_input_embeddings().weight.size(0) != len(
        tokenizer
    ):
        base_model.resize_token_embeddings(len(tokenizer))

    new_model = PeftModel.from_pretrained(
        base_model, args.lora_model, device_map="auto", torch_dtype="auto"
    )
    new_model.eval()
    ...
    if args.hf_hub_model_id:
        base_model.push_to_hub(
            args.hf_hub_model_id, token=args.hf_hub_token, max_shard_size="10GB"
        )
